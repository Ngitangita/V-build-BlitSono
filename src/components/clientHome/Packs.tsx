  ...
  return (
    <div
      className="w-10 text-center p-2 pt-0 text-5xl text-[#18769C]/50 relative 
                 bottom-20 left-[99%] bg-[#F3F4F6] hover:bg-[#1E2939] hover:text-[#18769C] 
...
function PrevArrow({ onClick }: { onClick?: () => void }) {
  return (
    <div
      className="w-10 text-center p-2 pt-0 text-5xl text-[#18769C]/50 relative 
                 right-10 top-20 bg-[#F3F4F6] hover:bg-[#1E2939] hover:text-[#18769C] 
                 rounded cursor-pointer z-10"
      onClick={onClick}
    >
      ‹
...
export default function Packs({ packItems, autoScrollIntervalMs = 5000 }: Props) {
  const settings = {
    dots: false,
    infinite: true,
    speed: 500,
    ...
    <div className="px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
      <div className="text-[#18769C] w-full flex flex-col gap-4 pt-6 sm:pt-10">
        ...
        <p className="italic text-xs sm:text-sm lg:text-base">
          <FaHandPointRight size={18} className="inline mr-2 text-[#1E2939]" />
      ...
      <div className="relative pt-4 sm:pt-6 pb-6 pr-4 sm:pr-10 pl-4 sm:pl-10">
        <Slider {...settings}>
          {packItems.map((m) => (
            <div key={m.id_bundle} className="px-1 sm:px-2">
              <Link
                to="/pack-materiels"
                className="block w-full h-full"
              >
                <div className="rounded-lg p-3 sm:p-4 shadow-md hover:scale-105 transition duration-300 ease-in-out bg-white hover:shadow-lg group">
                  ...
                  <p className="text-xs sm:text-sm text-gray-700 mt-1 group-hover:text-[#18769C]">
                    Prix : <strong>{m.daily_price} Ar</strong>
                  </p>
                </div>
              </Link>
            </div>
